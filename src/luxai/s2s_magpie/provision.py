"""Provision the immutable model assets required by the S2S runtime."""
...
from speech_to_speech.model_assets import load_asset_lock
...
def load_lock() -> dict[str, Any]:
    """Load the model asset lock bundled with the installed package."""

    return load_asset_lock()
...
def provision_huggingface(
    ...
    *,
    ...
    for name, asset in lock["huggingface"].items():
        ...
        action = "verified" if verify_only else "provisioned"
        print(f"{action} hf:{name} -> {', '.join(resolved)}")
